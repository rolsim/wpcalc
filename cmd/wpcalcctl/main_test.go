...
func TestUserLifecycleAndGrant(t *testing.T) {
	...
	// The seeded tenant's id is generated per database, so it is looked up
	// rather than spelled as "1" on the command line.
	tenantID := defaultTenant(t, db).String()
	if err := run([]string{"user", "grant", "bob", "-tenant", tenantID, "-role", "mandant_admin"}); err != nil {
	...
	if err := run([]string{"user", "revoke", "bob", "-tenant", tenantID}); err != nil {
...
func itoa(v uuid.UUID) string {
	return v.String()
}

// defaultTenant is the tenant a fresh database seeds; its id is generated by
// the migration, so tests look it up by name.
