...
// defaultTenantID is the "Default" tenant every fresh migrated database seeds
// — every test here operates within it unless it says otherwise. Its id is
// generated by the migration rather than fixed, so it has to be looked up:
// there is no well-known tenant id any more.
func defaultTenantID(t *testing.T, db *store.DB) uuid.UUID {
	t.Helper()
...
// withActiveTenant fills in a stub identity's active tenant once the database
// exists. Stub identities are built by the test before newTestServer opens the
// database, so they cannot name the seeded tenant themselves.
func withActiveTenant(t *testing.T, db *store.DB, authn auth.Authenticator) auth.Authenticator {
	t.Helper()
	sa, ok := authn.(stubAuth)
	if !ok || sa.id.IsZero() || sa.id.ActiveTenantID != nil {
		return authn
	}
	id := defaultTenantID(t, db)
	sa.id.ActiveTenantID = &id
	return sa
}
...
func newTestServer(t *testing.T, authn auth.Authenticator) *testServer {
	...
	authn = withActiveTenant(t, db, authn)
...
func (ts *testServer) employee(t *testing.T, name, start, end string) uuid.UUID {
	t.Helper()
	e := domain.Employee{TenantID: ts.defaultTenant(t), DisplayName: name}
...
func TestGridRendersMonthWithEmployeesAndTotals(t *testing.T) {
	...
	if err := ts.db.SetHours(t.Context(), ts.defaultTenant(t), alice, day, 775); err != nil {
		t.Fatal(err)
	}
...
func TestSetHoursJSONReturnsServerComputedTotals(t *testing.T) {
	...
	day, _ := domain.ParseDate("2026-07-14")
	if err := ts.db.SetHours(t.Context(), ts.defaultTenant(t), bob, day, 200); err != nil {
		t.Fatal(err)
	}
...
func TestNoPageRendersAnUntranslatedMarker(t *testing.T) {
	...
	day, _ := domain.ParseDate("2026-07-14")
	if err := ts.db.SetHours(t.Context(), ts.defaultTenant(t), id, day, 775); err != nil {
		t.Fatal(err)
	}
	if err := ts.db.SetDayComment(t.Context(), ts.defaultTenant(t), day, "Notiz"); err != nil {
		t.Fatal(err)
	}
...
// itoa kept its name; ids are text now, so it just renders one.
func itoa(v uuid.UUID) string { return v.String() }
...
// defaultTenant is the seeded tenant this server's database starts with.
func (ts *testServer) defaultTenant(t *testing.T) uuid.UUID {
	t.Helper()
	return defaultTenantID(t, ts.db)
}
