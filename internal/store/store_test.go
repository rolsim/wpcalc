...
// defaultTenant is the tenant every fresh database is seeded with. Its id is
// generated by the migration rather than fixed, so tests look it up by name
// instead of hardcoding one — there is no well-known id any more, which is the
// point.
func defaultTenant(t *testing.T, db *DB) uuid.UUID {
	t.Helper()
...
func mustEmployee(t *testing.T, db *DB, name, start, end string) uuid.UUID {
	t.Helper()
...
func TestOpenCreatesDatabaseFile(t *testing.T) {
	...
	if _, err := db.Employees(t.Context(), defaultTenant(t, db)); err != nil {
...
func TestMigrationsUpDownUpIsClean(t *testing.T) {
	...
	// Captured up front: once the down migration has run there is no tenants
	// table left to look the Default tenant up in, and the assertions below
	// still need an id to query with.
	tenantID := defaultTenant(t, db)
	id := mustEmployee(t, db, "Before", "2026-01-01", "")
	if err := db.SetHours(ctx, tenantID, id, mustDate(t, "2026-07-14"), 775); err != nil {
	...
	if _, err := db.Employees(ctx, tenantID); err == nil {
	...
	// A fresh up seeds a *new* Default tenant, with a new id — the old one is
	// gone with the rest of the data, which is exactly what is being asserted.
	emps, err := db.Employees(ctx, defaultTenant(t, db))
	if err != nil {
	...
	if err := db.SetHours(ctx, defaultTenant(t, db), newID, mustDate(t, "2026-07-14"), 800); err != nil {
...
func TestEntryRejectedBeforeStartDate(t *testing.T) {
	...
	err := db.SetHours(t.Context(), defaultTenant(t, db), id, mustDate(t, "2026-07-13"), 800)
	...
	if err := db.SetHours(t.Context(), defaultTenant(t, db), id, mustDate(t, "2026-07-14"), 800); err != nil {
...
func TestEntryRejectedAfterEndDate(t *testing.T) {
	...
	err := db.SetHours(t.Context(), defaultTenant(t, db), id, mustDate(t, "2026-07-21"), 800)
	if !errors.Is(err, domain.ErrNotEmployed) {
		t.Fatalf("SetHours the day after end: got %v, want ErrNotEmployed", err)
	}

	if err := db.SetHours(t.Context(), defaultTenant(t, db), id, mustDate(t, "2026-07-20"), 800); err != nil {
...
func TestSetHoursIsIdempotentAndClears(t *testing.T) {
	...
	if err := db.SetHours(ctx, defaultTenant(t, db), id, day, 775); err != nil {
	...
	if err := db.SetHours(ctx, defaultTenant(t, db), id, day, 800); err != nil {
	...
	if err := db.SetHours(ctx, defaultTenant(t, db), id, day, 0); err != nil {
...
func TestSetHoursRejectsOutOfRange(t *testing.T) {
	...
	for _, v := range []domain.Centihours{-1, 2401, 10000} {
		if err := db.SetHours(t.Context(), defaultTenant(t, db), id, day, v); !errors.Is(err, domain.ErrHoursRange) {
...
func TestSetHoursRejectsAnEmployeeFromAnotherTenant(t *testing.T) {
	...
	day := mustDate(t, "2026-07-14")
	err = db.SetHours(ctx, defaultTenant(t, db), empB, day, 800)
...
func TestTotalsEqualSumOfEntries(t *testing.T) {
	...
	written := map[uuid.UUID]map[domain.Date]domain.Centihours{alice: {}, bob: {}}
	...
	for i, day := range july.Days() {
		for j, emp := range []uuid.UUID{alice, bob} {
			v := values[(i+j)%len(values)]
			if err := db.SetHours(ctx, defaultTenant(t, db), emp, day, v); err != nil {
				t.Fatalf("SetHours: %v", err)
			}
			written[emp][day] = v
			want += v
...
func TestTotalsIgnoreEntriesOutsideTheMonth(t *testing.T) {
	...
	for _, d := range []string{"2026-06-30", "2026-07-01", "2026-07-31", "2026-08-01"} {
		if err := db.SetHours(ctx, defaultTenant(t, db), id, mustDate(t, d), 100); err != nil {
			t.Fatalf("SetHours(%s): %v", d, err)
		}
	}

	totals, err := db.Totals(ctx, defaultTenant(t, db), domain.NewYearMonth(2026, time.July))
	if err != nil {
		t.Fatalf("Totals: %v", err)
	}
	if totals.Grand != 200 {
...
func TestEmployeesActiveInMatchesDomainRule(t *testing.T) {
	...
	byID := make(map[uuid.UUID]domain.Employee)
	for _, s := range specs {
	...
	for _, m := range []domain.YearMonth{
		...
		fromSQL, err := db.EmployeesActiveIn(ctx, defaultTenant(t, db), m)
		if err != nil {
			t.Fatalf("EmployeesActiveIn(%s): %v", m, err)
		}
		got := make(map[uuid.UUID]bool, len(fromSQL))
...
func TestDayComments(t *testing.T) {
	...
	if err := db.SetDayComment(ctx, defaultTenant(t, db), day, "Betriebsausflug"); err != nil {
		t.Fatalf("SetDayComment: %v", err)
	}
	comments, err := db.DayComments(ctx, defaultTenant(t, db), july)
	if err != nil {
	...
	if err := db.SetDayComment(ctx, defaultTenant(t, db), day, "Ersetzt"); err != nil {
		t.Fatalf("SetDayComment replace: %v", err)
	}
	comments, _ = db.DayComments(ctx, defaultTenant(t, db), july)
	...
	if err := db.SetDayComment(ctx, defaultTenant(t, db), day, "   "); err != nil {
...
func TestEmployeeRangeTotalAndEntries(t *testing.T) {
	...
	for _, spec := range []struct {
		...
		if err := db.SetHours(ctx, defaultTenant(t, db), id, mustDate(t, spec.day), spec.v); err != nil {
			t.Fatalf("SetHours: %v", err)
		}
	}
...
func TestEmployeeCRUD(t *testing.T) {
	...
	if _, err := db.Employee(ctx, uuid.NewV4()); !errors.Is(err, ErrNotFound) {
		t.Errorf("missing employee: got %v, want ErrNotFound", err)
	}
	if err := db.UpdateEmployee(ctx, domain.Employee{
		ID: uuid.NewV4(), TenantID: defaultTenant(t, db), DisplayName: "Ghost", StartDate: mustDate(t, "2026-01-01"),
	}); !errors.Is(err, ErrNotFound) {
		t.Errorf("updating missing employee: got %v, want ErrNotFound", err)
	}
	if err := db.DeleteEmployee(ctx, uuid.NewV4()); !errors.Is(err, ErrNotFound) {
...
func TestCreateEmployeeRejectsInvalid(t *testing.T) {
	db := testDB(t)
	bad := []domain.Employee{
		{TenantID: defaultTenant(t, db), DisplayName: "", StartDate: mustDate(t, "2026-01-01")},
		{TenantID: defaultTenant(t, db), DisplayName: "NoStart"},
		{
			TenantID:    defaultTenant(t, db),
		...
		{DisplayName: "NoTenant", StartDate: mustDate(t, "2026-01-01")}, // TenantID left nil
	}
	for _, e := range bad {
